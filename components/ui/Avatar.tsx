...
function Avatar({ variant, content, class: _class = "", ...btnProps }: Props) {
  ...
  if (variant === "abbreviation") {
    return (
      <button
        {...btnProps}
        class={`text-caption font-caption rounded-full p-4 outline-solid outline-offset w-6 h-6 flex justify-center items-center hover:bg-interactive hover:border-interactive hover:text-default-inverse disabled:bg-interactive disabled:text-default-inverse disabled:border-interactive ${_class}`}
      >
